# 华为云数据智能产品方案：先跑通客户任务，再选择组织与技术架构

> 本文是基于公开资料形成的产品设计建议，不代表华为云现状、立项决定或交付承诺；内部接口、账号、容量、成本与团队资源仍未验证。

<details>
<summary>文档说明、建议工作名与证据标签</summary>

- 方案基准日：2026-08-20
- 建议工作名：`Data Intelligence Agent`，不是已确认的正式产品名；命令行示例统一使用占位命令 `di`
- 核心目标：**让用户或外部 Agent 用一句话完成接入数据、生成数据产品、管理数据、查数、分析、建知识、查数据库和受控行动；优先跑通“接入—资产化—使用—治理”的完整数据生命周期，而不是只做运维咨询。**
- 主要读者：产品负责人、架构与研发负责人、数据库和 DataArts 相关 Owner；业务读者可先读[读者指南](./start-here.md)
- 实现原则：先确定客户能感知的任务和交付物，再选择组织与架构。Agent 执行框架（下文简称 `Harness`）、任务目标合同（`Goal Contract`）以及 CLI/MCP 等接口只负责后台执行，不替代产品能力定义。
...
> “最近 30 天华东退款率为什么上升？排除订单规模变化，并说明哪些是事实、哪些还只是推断。”

今天，这个问题通常要跨业务、数据和数据库团队：确认指标口径、找表、申请权限、写 SQL、检查数据质量、做贡献分析，再把结果整理成报告。目标产品要把这条链路接起来，而不是只把最后一步换成聊天：

1. 识别用户、数据范围、退款率口径和比较周期；
2. 有歧义先追问，无权限明确拒绝，数据过期或缺失时停止并说明；
3. 执行受控查询与分析，保留 SQL、参数、数据版本和查询 ID；
4. 返回表、图和可复算报告，把事实、候选解释和 `Unknown` 分开；
5. 若怀疑数据库异常，引用实例、SQL、计划和变更证据，不把时间相关性直接写成根因；
6. 若用户要求执行动作，先展示影响和审批，执行后再由权威系统验证结果。

用户购买的是这条完整任务。`Harness`、状态机、CLI/MCP 和内部模块只有在它们让任务更可靠时才有产品价值。

### 0.2 本文术语身份

| 术语 | 身份 | 本文含义 |
|---|---|---|
| Data Intelligence / 数据智能 | 建议产品工作名 | 统一承载数据接入、问数、分析、知识、治理和数据库调查的能力集合 |
| DataArts Intelligence | 建议产品名 | 公司级协作方案的产品名称候选，不是现有正式名称 |
| Database Intelligence | 建议产品名 | 数据库团队低依赖方案的名称候选，不是现有正式名称 |
| C0–C7 | 本文定义 | 八类产品能力编号，用于讨论范围和优先级，不是行业标准 |
| Goal Contract | 本文定义 | 一次任务的目标、边界、成功条件和停止条件 |
| Artifact / Evidence | 本文执行合同术语 | 可继续使用的任务结果，以及支持结论和状态的证据 |
| Gate | 本方案的验证关口 | 拿到指定证据后才能继续的决策点，不是现有云功能 |
| Harness、CLI、MCP、API | 行业技术或接口形式 | 后台执行与外部 Agent 调用方式，不是业务用户要购买的功能 |

更完整的术语说明见[读者指南](./start-here.md)。

### 0.3 先定产品能力目标，再选方案和架构
...
4. **技术细节**：最后再看执行框架、任务状态、部署、Agent 接口、语义、自然语言转 SQL、数据库调查和评测。

两种方案使用同一组客户能力，但首期重点不同：方案一先做多源接入、数据产品、问数分析和治理；方案二先围绕一个数据库完成接入、资产化、数据库调查和问数。业务人员都从统一对话入口开始；程序和外部 Agent 再通过 CLI、MCP 或 API 调用同一能力。两种方案必须共享任务状态、结果、证据、权限和评测，不能各造一套合同。

### 0.4 当前决策建议

| 决策项 | 方案一：华为云竞争力优先 | 方案二：数据库部门绿地突围 |
|---|---|---|
| 产品内核 | 同一套任务、状态、结果、证据、权限和外部调用合同 | 同一套，不另造第二种架构 |
...
`建议`：不要等组织讨论结束才开始验证。技术小队先按方案二搭建低依赖纵向样例，同时给方案一 2–4 周书面 Gate。若公司级 Owner、共同 OKR、能力包合同和发布权能够落实，同一套代码和合同升格为方案一；条件不成立，则由数据库团队继续负责端到端闭环。
...
本文将八类客户能力编号为 C0–C7，只为后续讨论范围、依赖和优先级。这些编号不是行业分类，也不应直接成为客户首页菜单。
...
为了避免八个名称看起来彼此重叠，再补充各能力的首要使用者和边界：

| 能力 | 首要用户与原始问题 | 与相邻能力的区别 |
|---|---|---|
| C0 数据接入与准备 | 数据工程师需要把数据库或文件稳定变成可用数据 | 负责数据怎样到达、何时到达；不负责定义业务指标 |
| C1 智能问数 | 业务人员需要一个口径明确、可核验的数值答案 | 负责回答“是多少”；复杂对比和原因分析交给 C2 |
| C2 智能分析 | 业务分析人员需要解释变化、分群和贡献 | 负责回答“发生了什么、可能为什么”；不把相关性写成因果 |
| C3 数据知识构建 | 数据 Owner 需要把表、指标、关系和规则整理成可审核资产 | 负责形成可复用语义和知识草稿；发布仍需 Owner 审核 |
| C4 数据库洞察 | DBA/SRE 需要用实例权威证据调查性能和变更影响 | 负责数据库运行状态；不代替跨域业务分析 |
| C5 智能治理 | 治理和安全负责人需要发现敏感、质量、血缘和影响问题 | 负责策略、质量和生命周期；不代替数据接入执行 |
| C6 安全行动 | 值班人员需要在明确对象上执行可审批、可验证的动作 | 负责少量受控变更；不允许模型自行扩大动作范围 |
| C7 主动智能 | 业务或运维 Owner 希望系统持续监测并主动发起调查 | 负责触发、订阅和建议；依赖前述能力已经稳定 |

#### 首期投资顺序

| 方案 | 第一批必须跑通 | 第二批补齐 | 后置 |
|---|---|---|---|
| 公司级协作方案 | C0 接入、C3 知识草稿、C1 问数 | C2 分析、C5 治理最小闭环、C4 数据库调查 | C6 安全行动、C7 主动智能 |
| 数据库团队方案 | C4 数据库调查、C0 接入、C1 问数、C3 知识草稿 | C2 基础分析、C5 治理最小闭环 | C6 安全行动、C7 主动智能 |

第一条样例统一使用“接订单库 → 生成资产草稿 → 问退款率 → 调查数据库”这条故事，不按八个能力分别做八个 Demo。

<details>
<summary>查看内部设计分、权重和完整排序</summary>

下面的数字只用于讨论投资先后，不是华为云现状能力分，也不是客户调研或账号实测结果。
