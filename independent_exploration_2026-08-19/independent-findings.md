...
这份报告想回答一个实际决策：当业务负责人问“最近 30 天华东退款率为什么上升”，华为云现有产品能否像一个完整产品那样找到数据、使用统一口径、完成分析并交付可核验结果；如果不能，应该补现有产品、由数据库团队另做闭环，还是采用联合方案。

三分钟内先记住四点：

| 问题 | 当前回答 |
|---|---|
| 华为云是不是完全没有数据智能能力 | 不是。DataArts、Insight、Fabric、LakeFormation 和数据库产品各自已有重要能力 |
| 公开资料是否已经证明它们是一套贯通产品 | 没有。共享对象、身份、语义、会话、评测和发布生命周期仍是 `Unknown` |
| Databricks 的公开优势只是聊天入口吗 | 不是。更关键的是数据、语义、权限、查询、评测和外部调用之间的对象连续性 |
| 现在应该直接决定产品归属吗 | 不应该。先用合成数据执行 G1–G5，再按结果选择增强、联合、独立或联邦方案 |

当前最稳健的候选是“双平面、一条客户任务”：数据平台负责跨域数据、语义、分析和评测；数据库产品负责实例上下文、安全执行和运维证据；两者使用同一身份映射、任务状态和证据合同。它仍是待验证建议，不是组织结论。

不熟悉文中术语时，先看 [`start-here.md`](./start-here.md)；只想了解五条客户任务，可直接跳到[第 6 节](#five-customer-task-chains)。
...
<a id="five-customer-task-chains"></a>
